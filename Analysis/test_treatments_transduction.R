...
p_phage_ery = ggplot(results) +
  ...
  scale_x_continuous(breaks=seq(0,max(results$time),4))+
  theme_bw() +
  labs(y = "cfu or pfu per mL", x = "Time (hours)", colour = "Organism:", title = "Phage and ery") +
  scale_colour_manual(breaks = c("Be", "Bt", "Bet", "Pl"),
                      values = c("#685cc4","#6db356","#c2484d","#c88a33"),
                      labels = c(expression(B[E]),
                                 expression(B[T]),
...
p_phage_tet = ggplot(results) +
  ...
  scale_x_continuous(breaks=seq(0,max(results$time),4))+
  theme_bw() +
  labs(y = "cfu or pfu per mL", x = "Time (hours)", colour = "Organism:", title = "Phage and tet") +
  scale_colour_manual(breaks = c("Be", "Bt", "Bet", "Pl"),
                      values = c("#685cc4","#6db356","#c2484d","#c88a33"),
                      labels = c(expression(B[E]),
                                 expression(B[T]),
...
plot_grid(plot_grid(p_phage + theme(legend.position = "none"),
          p_both + theme(legend.position = "none"),
          p_phage_ery + theme(legend.position = "none"),
          p_phage_tet + theme(legend.position = "none"), nrow = 2),
          plot_grid(NULL, legend, NULL,
                    nrow = 1),
          nrow = 2,
          rel_heights = c(1,0.05))

ggsave(here::here("Figures", "treatments_tr.png"))
