...
p_duration = ggplot(all_results) +
  ...
  geom_point(data = all_results %>% filter(pha_con == 1e+08) %>% filter(diff %in% c(0,3,6,9)),
...
pb = plot_grid(p_remain + theme(legend.position = "none"),
               ...
               get_legend(p_remain + guides(colour = F) + theme(legend.position = "bottom")),
               ncol = 1,
...
p_abx1_pha1 = ggplot(results) +
  ...
  annotate("segment", y = 10^(log10(max(results$Bet, na.rm = T))+1),
           yend = max(results$Bet), x = 36, xend = 36) +
  geom_label(y = log10(max(results$Bet, na.rm = T))+1,
             x = 36, label = "Max DRP", size = 3) +
...
plot_grid(pb,
          ...
          plot_grid(
            plot_grid(p_abx1_pha1 + theme(legend.position = "none"),
                      p_abx4_pha1 + theme(legend.position = "none"),
                      p_abx7_pha1 + theme(legend.position = "none"),
                      p_abx10_pha1 + theme(legend.position = "none"),
                      ncol = 2),
...
ggsave(here::here("Figures", "new_fig5.png"), height = 12, width = 14)
